...
import ctwedge.fmtester.experiments.MutationScore.AddedFeatureTreatment;
...
public class SpecificityChecker {
	...
	public SpecificityChecker(IFeatureModel oldFm, IFeatureModel newFm, boolean useEnum) {
		this.oldFm = oldFm;
		this.newFm = newFm;
	...
	public boolean isSpecific(Test t) {
		...
		Boolean validInOld = useEnum ? MutationScore.isTestValidEnum(oldFm, t)
				: MutationScore.isTestValidBool(oldFm, t);
		...
		Boolean validInNew = useEnum ? MutationScore.isTestValidEnum(newFm, t)
				: MutationScore.isTestValidBool(newFm, t);
		assert validInNew : fmautorepair.utils.Utils.getFeatureNames(newFm) + "\n" + t;

		return validInNew != validInOld;
	}

	public boolean isSpecificWithBooleans(Test t) {
		// in old missing features are just skipped since some features may be added
		MutationScore.treat_missing_feature_as = MissingFeatureTreatment.SKIP;
		MutationScore.treat_added_feature_as = AddedFeatureTreatment.UNSELECTED;
		Boolean validInOld = MutationScore.isTestValidBool(oldFm, t);
